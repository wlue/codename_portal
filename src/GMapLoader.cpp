...
void GMapLoader::LoadLevel(GLevel* level)
{
	ClearData();
	filename = level->getFileDir();
...
void GMapLoader::ParseTile(string& objtype, string& tiletype, string parse)
{
	objtype = parse.substr(0, parse.find('('));
	tiletype = parse.substr(parse.find('(') + 1, parse.length() - objtype.length() - 2);
}

void GMapLoader::ParseAttributes(string& tiletype, string& tileatr, string tileinfo, char delim)
{
	...
	for(int y=0; y < (int) map_maxy; ++y)
	{
		for(int x=0; x < (int) map_maxx; ++x)
		{
			...
			for(tilesItr = tiles.begin(); tilesItr != tiles.end(); ++tilesItr)
			{
				...
				if(objtype == "terrain")	newObj = new CTerrain(tiletype, x, y);
				if(objtype == "door")		newObj = new CDoor(x, y);
				...
				if(newObj != NULL)
				{
					if(!tileatr.empty())
					{
						...
						vector<string>::iterator setItr;
						SplitValues(settings, tileatr, ';');
						for(setItr = settings.begin(); setItr != settings.end(); ++setItr)
						{
							string setting = *setItr, left, right;
							GetSides(left, right, setting, "=");
							if(!left.empty() && !right.empty())
								newObj->SetAttribute(left, right);
						}
					}
				}
				else
